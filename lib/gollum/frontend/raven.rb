...
class Raven
	...
	DEFAULT_PUBLIC_KEY_FILE = File.expand_path(File.dirname(__FILE__))+'/pubkey2.pem'
	...
	def initialize
		...
		if @@publickey == nil
			@@publickey = Hash.new
			begin
				@@publickey[ DEFAULT_PUBLIC_KEY_ID ] = OpenSSL::PKey::RSA.new File.read DEFAULT_PUBLIC_KEY_FILE
			rescue => err
				$stderr.puts err
	...
	def authenticate( cgi, message = @message, iact = @iact )
		...
		response = check_session( session, iact ) || check_response_from_raven(session, iact ) || get_raven_request(session, message, iact )
	...
	def self.check_session( session, iact )
	...
	def check_response_from_raven( params, session, iact )
		return nil if params['WLS-Response'] == ""
	...
	def get_raven_request(session, message, iact)
		...
		# Return the raven redirection url
		return "#{@raven_url}?ver=#{CGI.escape(@raven_version)};url=#{CGI.escape(@return_url)};desc=#{CGI.escape(@description)};msg=#{CGI.escape(message)};iact=#{CGI.escape(iact)};aauth=#{CGI.escape(@aauth.join(","))};params=#{CGI.escape(params)};fail=#{CGI.escape(@fail)}"
